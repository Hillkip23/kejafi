...
# FIX 4: decorated with @st.cache_data so load_zori() runs once per session.
# The original called load_zori() inside the function body with no caching,
# meaning it re-read the CSV on every Streamlit re-render.
@st.cache_data
...
def push_property_to_api(
    ...
    # FIX 3: None numeric fields coerced to safe defaults before POST.
    # FastAPI validators reject null on numeric columns; pci_2023, pop_growth,
    # metro_elasticity, and risk_score can all be None when the metro profile
    # is unavailable.
    payload = {
        "id": prop_id,
...
# ----------------------------------------------------------
# FIX 5: read from namespaced keys
