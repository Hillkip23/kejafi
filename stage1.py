...
Fixes applied (v2):
1. POP_GROWTH_FALLBACK – provides realistic population growth rates for target metros
2. ZORI_PATH environment variable override for flexible deployment
3. Export preview in UI – shows market dict after simulation
4. Supply bucket mapping now uses adjustable thresholds (sidebar)
5. UTF-8 encoding, core guard, jump-diffusion, etc. (preserved from original)
"""
...
# Allow environment variable override for ZORI file location
ZORI_PATH_ENV = os.getenv("KEJAFI_ZORI_PATH")
if ZORI_PATH_ENV:
    ZORI_PATH = Path(ZORI_PATH_ENV)
else:
...
def load_zori():
    """
    Load Zillow Observed Rent Index (ZORI).
    Note: In production, consider moving this to a shared module
    (e.g., kejafi_shared.py) so Stage 2 can reuse the same cached DataFrame.
    """
...
# Population growth (2013-2017 → 2018-2022 approximate)
# Sources: Census Bureau / BLS / local market reports
POP_GROWTH_FALLBACK = {
    "Charlotte": 0.10,
    "Nashville": 0.12,
    "Atlanta": 0.08,
...
# =========================================================
# Stage 2 Export Pipeline (UPDATED)
# =========================================================
...
def export_metro_for_stage2(
    ...
    Args:
        ...
        supply_bucket_thresholds: Optional dict with 'very_inelastic', 'inelastic', 'elastic' thresholds
                                 Default: {'very_inelastic': 0.15, 'inelastic': 0.25, 'elastic': 0.40}
    
    Returns:
        Market dict compatible with Stage 2 PropertyValuation (includes pop_growth now)
    """
    ou = simulation_result.get("ou_params", {})
    county = county_info or COUNTY_DATA.get(metro_short_name, [{}])[0]
    
    # Use user-adjustable thresholds (from sidebar) or defaults
    if supply_bucket_thresholds is None:
        supply_bucket_thresholds = {
            "very_inelastic": 0.15,
            "inelastic": 0.25,
            "elastic": 0.40,
        }
    ...
    yoy_rent_growth = max(-0.02, min(theta / 12 * 12, 0.15))
    
    # Pop growth: use fallback dict if not provided by county_info
    pop_growth = POP_GROWTH_FALLBACK.get(metro_short_name, 0.05)
    ...
    return {
        ...
        "pop_growth": pop_growth,                     # FIXED: now populated
        "risk_score": risk_score,
        "risk_bucket": risk_bucket,
        # Stage 3 extension fields
...
def render_metro_card(
    ...
    with col:
        ...
        # NEW: Export to Stage 2 with preview
        with st.expander("📤 Export to Stage 2"):
            stage2_market = export_metro_for_stage2(
                short_name, result, county_info, supply_bucket_thresholds
            )
            ...
            st.caption("✅ This market data is automatically saved to session state and will be available in Stage 2.")
