...
def get_scenario_params(
    ...
    # FIX 3: CRYPTO_WINTER and STAGFLATION now include jump_intensity so the
    # jump-diffusion branch in run_simulation_standalone actually fires for
    # these scenarios in Stress mode. Values are intentionally modest since
    # the primary channel for these scenarios is cap rate / token discount,
    # not a sudden rent jump — but zero would silently skip JD entirely.
    #
    # FIX 5: CRYPTO_WINTER now includes vacancy_shock and cap_rate_shock keys
    # so downstream dict.get() calls never silently return None.
    base = {
        ...
        StressScenario.CRYPTO_WINTER: {
            # FIX 5: added vacancy_shock and cap_rate_shock (were missing entirely)
            "vacancy_shock": 0.02,
            "rent_growth_adj": 0.0,
...
def run_simulation_with_core(
    ...
    # FIX 2 (documented): this function hardcodes STANDARD + BASE_CASE —
    # caller (run_simulation) already guards: only invoked when
    # mode == EngineMode.STANDARD and use_core_for_standard() is True.
...
def run_simulation(
    ...
    # FIX 2: core path only fires in STANDARD mode — Stress Lab always uses
    # standalone to preserve jump-diffusion and stochastic cap rate models.
    ...
    with col:
        ...
        if "ou_params" in result:
            ...
            with st.expander("OU Parameters"):
                # FIX 1: Greek letters now render correctly (UTF-8 header above)
...
def main():
    ...
    # Store only an integer index in session state — never an enum instance.
    # Storing enum objects causes Streamlit's radio serializer to crash on
    # hot-reload when the freshly-constructed options list doesn't match the
    # stale stored value ("EngineMode.STRESS is not in iterable").
    ...
    # FIX 4: use_core defined unconditionally so the caption at the bottom
    # never raises NameError when HAS_CORE is False.
    use_core = False
    ...
    # FIX 4: use_core is always defined — no NameError possible here
    st.caption(
